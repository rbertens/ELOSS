...
void vishnu_profile_reader(const char* filename = "vishnu_profile.csv", const char* outputfile = "vishnu_profile.root") {
   ...
   TNtupleD *ntuple = new TNtupleD("ntuple","data from ascii file","x:y:tau:hydroneff:jewelneff");
   ...
   while (true) {
      ...
      ntuple->Fill(x,y,z,tau,hydroneff,jewelneff);
      nlines++;
   }
   printf("\n .. digested  %d points -- \n",nlines);
